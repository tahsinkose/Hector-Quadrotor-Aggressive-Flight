...
class AggressiveTrajectoryTracker {
 public:
  ...
  void run() {
    ...
    std::vector<geometry_msgs::Pose> trajectory = createPattern("SQUARE");
 ...
 private:
  ...
  std::tuple<std::vector<double>, std::vector<double>, std::vector<double>> VelocityPlanSweep(
    ...
    for (int i = 0; i < traj.size() - 1; i++) {
      ...
      double norm = sqrt(pow(traj[i].position.x - traj[i + 1].position.x, 2) +
                         pow(traj[i].position.y - traj[i + 1].position.y, 2));
